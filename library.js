...
for (idx = 0; idx < 6; idx++) {
    ...
    rowd1.innerText = idx + 1;
    let rowd2 = document.createElement("td");
    rowd2.innerText = table_data[idx]["Book"];
    let rowd3 = document.createElement("td");
    rowd3.innerText = table_data[idx]["Author"];
    let rowd4 = document.createElement("td");
    rowd4.innerText = table_data[idx]["Lender"];
    let rowd5 = document.createElement("td");
    rowd5.innerText = table_data[idx]["Borrower"];
...
borrow(user);
{
    for (idx = 0; idx < 6; idx++) {
        ...
        if (table_data[idx]["Borrower"] === user) {
            button.innerText = "Return";
            button.onclick = Return(this);
        }

        if (table_data[idx]["Lender"] != user) {
            if (
                !table_data[idx]["Actions"] ||
                (table_data[idx]["Actions"] && table_data[idx]["Borrower"] === user)
            )
